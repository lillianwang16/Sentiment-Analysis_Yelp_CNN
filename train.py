...
import nltk
import string
#from vectorize import vectorize_word
...
def train_cnn():
	...
	xx = []
        ...
        #for single_rev in x_raw:
        #    x.append(vectorize_word(single_rev))

        num_ele = len(x_raw)
        for i in range (0, num_ele):
            rev = x_raw[i]
            vec = vectorize_word(rev)
            if len(vec):
       		xx.append(vec)
                yy.append(y_raw[i])
		...
		with sess.as_default():
			...
			if params['word2vec']:
            		    # initial matrix with random uniform
           	       	    initW = np.random.uniform(-0.25,0.25,(len(vocab_processor.vocabulary_), params['embedding_dim']))
            		    # load any vectors from the word2vec
           		    print("Load word2vec file {}\n".format(params['word2vec']))
            		    with open(params['word2vec'], "rb") as f:
                		header = f.readline()
			...
			for train_batch in train_batches:
				...
				if current_step % params['evaluate_every'] == 0:
					...
					log_dev.write('{}'.format(dev_accuracy))
